...
<c:if test="${type.id==-3}">
    <jsp:include page="header.jsp?flag=-3"></jsp:include>
</c:if>
...
<c:if test="${not empty pno}">
        <div style="text-align:center;">
            <a class="btn btn-info"  href="${ctx}/goodslist?flag=list&pno=1&typeid=${type.id}">首页</a>
            <a class="btn btn-info" ${pno==1?"disabled":""} href="${ctx}/goodslist?flag=list&pno=${pno-1}&typeid=${type.id}">上一页</a>
            <h3 style="display:inline;">[${pno}/${pcount}]</h3>

            <a class="btn btn-info" href="${ctx}/goodslist?flag=list&pno=${pno+1}&typeid=${type.id}">下一页</a>
            <a class="btn btn-info" ${pno==pcount?"disabled":""} href="${ctx}/goodslist?flag=list&pno=${pcount}&typeid=${type.id}">尾页</a>
            <input type="text" class="form-control" style="display:inline;width:60px;" value="">
            <a class="btn btn-info" href="javascript:void(0);" onclick="location.href='${ctx}/goodslist?flag=list&pno='+(this.previousElementSibling.value)+'&typeid=${type.id}'">GO</a>
        </div>
</c:if>
    </div>
</div>
